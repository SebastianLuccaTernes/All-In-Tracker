...
test.describe('Database Integration Tests', () => {
  ...
  test('test add, update and delete the player integration', async ({ page }) => {
    ...
    // Verify player stats are updated in the database
    const updatedPlayer = await Player.findOne({ playerName: 'TestUser' }).exec();
    expect(updatedPlayer).not.toBeNull();
    expect(updatedPlayer.games).toBe(2);
    expect(updatedPlayer.boughtIn).toBe(60);
    expect(updatedPlayer.cashedOut).toBe(60);
    expect(updatedPlayer.wonOrLost).toBe(0);
